...
export function AcceptedOrder({ refreshKey }: ListOrderProps) {
    ...
    function UpdatetoDeliveryOrder(id: string) {
        const token = localStorage.getItem('token');
        if (!token) {
            console.error('No token found');
            return;
    ...
    const fetchOrders = async () => {
        try {
            ...
            if (!date) { 
                const response = await api_axios.get(`orders/by-status?status=ACCEPTED`, {
                    headers: {
                        Authorization: `Bearer ${token}`
                    }
                });
            ...
            const response = await api_axios.get(`orders/by-delivery-date?date=${formattedDate}&status=ACCEPTED`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
    ...
    function navigatetoNumber(order: Order) {
        // Check if the order and its userClient exist and navigate to the WhatsApp link
        if (order && order.userClient && order.userClient.phone) {
            window.location.href = `https://wa.me/${order.userClient.phone}`;
        }
    }
    ...
    const columns: ColumnDef<Order>[] = useMemo(() => [
        ...
        },
        {
            ...
            header: "Telefone",
            cell: info => {
                const order = info.row.original; // Get the order object
                return (
                    <div className="flex items-center space-x-3">
                        <span>{info.getValue() as React.ReactNode}</span>
                        <button onClick={() => navigatetoNumber(order)} className="text-blue-500 hover:text-blue-700">
                            <BsFillChatTextFill />
                        </button>
                    </div>
                );
                ...
                <AlertDialogContent>
                    ...
                    <AlertDialogFooter>
                        ...
                        <AlertDialogAction
                            onClick={() => {
                                if (selectedOrder) {
                                    UpdatetoDeliveryOrder(selectedOrder.id);
                                }
