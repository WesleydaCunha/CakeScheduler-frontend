...
export function DeliveredOrder({ refreshKey }: ListOrderProps) {
    ...
    const fetchOrders = async () => {
        try {
            ...
            if (!date) {
                const response = await api_axios.get(`orders/by-status?status=DELIVERY`, {
                    headers: {
                        Authorization: `Bearer ${token}`
                    }
                });
            ...
            const response = await api_axios.get(`orders/by-delivery-date?date=${formattedDate}&status=DELIVERY`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
